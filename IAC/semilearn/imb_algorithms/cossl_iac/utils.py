...
def classifier_warmup(args, model, train_labeled_set, train_unlabeled_set, N_SAMPLES_PER_CLASS, num_class, gpu):
    ...
    print('    Total params: %.2fM' % (sum(p.numel() for p in model.classifier.parameters()) / 1000000.0))
...
def TFE(args, labeled_loader, unlabeled_loader, tfe_model, num_classes, num_samples_per_class, gpu):
    ...
    with torch.no_grad():
        ...
        target_stack = torch.zeros(len(labeled_target_stack), num_classes).cuda(gpu).scatter_(1, labeled_target_stack.view(-1, 1).type(torch.int64), 1)
...
def weight_imprint(args, model, labeled_set, num_classes, gpu):
    ...
    new_weight = torch.zeros(num_classes, model.backbone.num_features)

    for i in range(num_classes):
        tmp = output_stack[target_stack == i].mean(0)
        new_weight[i] = tmp / tmp.norm(p=2)
    model.classifier = torch.nn.Linear(model.backbone.num_features, num_classes, bias=False).cuda(gpu)
        # model.classifier.reset_parameters()
    model.classifier.weight.data = new_weight.cuda(gpu)
...
def classifier_train(args, labeled_trainloader, model, optimizer, scheduler, ema_optimizer, val_iteration, gpu):
    ...
    for batch_idx in range(val_iteration):
        ...
        outputs = model.classifier(inputs_x)
