...
class AlgorithmBase:
    ...
    def evaluate(self, eval_dest='eval', out_key='logits', return_logits=False):
        ...
        with torch.no_grad():
            for data in eval_loader:
                ...
                if saved_features is not None:
                    saved_features = torch.cat((saved_features, self.model(x)['feat']), 0)
                else:
                    saved_features = self.model(x)['feat']
                if saved_ids is not None:
                    saved_ids = torch.cat((saved_ids, y), 0)
                else:
                    saved_ids = y
        ...
        np.save(self.save_dir+'/'+self.save_name+'/features_new.npy', saved_features.detach().cpu().numpy())
        np.save(self.save_dir+'/'+self.save_name+'/results_new.npy', saved_ids.detach().cpu().numpy())
